        ...
        <div class="container mx-auto px-4 py-6">
          ...
          <div class="mb-6 flex justify-end space-x-4">
            <input
                ...
                type="text"
                placeholder="Search tasks with status..."
...
const fetchTasks = async (url = '/api/tasks/tasks') => {
  try {
    const response = await axios.get(url, {
      ...
      params: {
        priority: currentFilter.value,
        status: searchKeyword.value,
      },
    });
...
const filteredTasks = computed(() => {
  ...
  // Apply search filter if searchKeyword is not empty
  if (searchKeyword.value) {
    filtered = filtered.filter(task => task.status.toLowerCase().includes(searchKeyword.value.toLowerCase()));
  }
...
// Search tasks based on the keyword
const searchTasks = async () => {
  if (!searchKeyword.value) {
    currentFilter.value = ''; // Reset any existing filters if the search term is cleared
  }
